...
export default function NewPatientOfferPage() {
  return (
    <main className="bg-white pb-28 md:pb-0">
      ...
      <section className="bg-light-gray py-16 text-charcoal sm:py-24">
        <div className="mx-auto grid max-w-7xl gap-8 px-6 sm:px-8 lg:grid-cols-[0.7fr_1.3fr] lg:items-stretch lg:gap-10 lg:px-16">
          <div className="rounded-2xl bg-navy p-8 text-white shadow-[0_18px_45px_-28px_rgba(15,35,60,0.7)] sm:p-10 lg:p-12"><p className="text-xs font-bold uppercase tracking-[0.2em] text-accent">New patient offer</p><p className="mt-3 text-8xl font-bold leading-none tracking-[-0.07em] text-white sm:text-9xl">$49</p><p className="mt-3 text-sm font-semibold uppercase tracking-[0.2em] text-[#c9d8eb]">First Move Session</p><p className="mt-6 max-w-sm text-lg leading-relaxed text-white/70">No guessing. No generic protocol. Start with clarity.</p></div>
          <div className="rounded-2xl bg-light-gray py-2 lg:py-4"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-accent">Your first visit includes</p><h2 className="mt-4 text-3xl font-bold tracking-tight text-navy sm:text-4xl">More Than an Adjustment.</h2><div className="mt-8 grid gap-x-8 gap-y-4 sm:grid-cols-2">{["Whole-body movement assessment", "Clinical evaluation", "Explanation of findings", "Personalized recommendations", "Hands-on treatment when clinically appropriate", "A clear next step"].map((item) => <div key={item} className="flex gap-3 border-t border-navy/15 pt-4 text-sm text-steel"><Check className="size-4 shrink-0 text-accent" aria-hidden="true" />{item}</div>)}</div><div className="mt-9"><CTAButton campaign="new-patient-offer" treatment={6} label="Book My $49 First Visit" variant="primary" size="lg" showArrow /></div></div>
        </div>
      </section>
