      ...
      <SectionWrapper>
        ...
        <div className="grid grid-cols-5 gap-px bg-border rounded-xl overflow-hidden mt-10 max-w-4xl mx-auto">
          {MOVE_METHOD_STEPS.map((s) => (
            <div key={s.step} className="bg-white p-3 text-center">
              <span className="text-xs font-bold text-softblue block">{s.step}</span>
              <p className="text-xs font-semibold text-charcoal mt-2 leading-tight">{s.title}</p>
            </div>
          ))}
        </div>
      </SectionWrapper>
